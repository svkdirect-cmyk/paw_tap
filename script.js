class DarkPawsClicker {
    ...
    updateComboCards() {
        console.log('Updating combo cards...');
    ...
    showCriticalEffect(points) {
        ...
        // Создаем контейнер для частиц поверх всего
        let particlesContainer = document.getElementById('critical-particles-container');
        if (!particlesContainer) {
            particlesContainer = document.createElement('div');
            particlesContainer.id = 'critical-particles-container';
            particlesContainer.className = 'critical-particles-container';
            document.body.appendChild(particlesContainer);
        }
        
        // Создаем частицы для текста "CRIT!"
        this.createTextParticles('CRIT!', container, particlesContainer);
        
        // Создаем частицы для очков
        this.createNumberParticles(`+${points}`, container, particlesContainer);
        
        // Удаляем элементы через время
        setTimeout(() => {
    ...
    createTextParticles(text, sourceContainer, targetContainer) {
        const rect = sourceContainer.getBoundingClientRect();
        const centerX = rect.left + rect.width / 2;
        const centerY = rect.top + rect.height / 2;
        
        // Создаем частицы для каждой буквы
        const letters = text.split('');
        const letterSpacing = 30;
        
        letters.forEach((letter, index) => {
            const offsetX = (index - (letters.length - 1) / 2) * letterSpacing;
            
            // Создаем несколько частиц для каждой буквы
            const particlesPerLetter = 3;
            ...
            for (let i = 0; i < particlesPerLetter; i++) {
                setTimeout(() => {
                    ...
                    // Случайное направление и вращение
                    const angle = Math.random() * Math.PI * 2;
                    ...
                    // Удаляем частицу после анимации
                    setTimeout(() => {
                        if (particle.parentNode === targetContainer) {
                            targetContainer.removeChild(particle);
                        }
                    }, 2000);
                }, i * 50);
            }
    ...
    createNumberParticles(numberText, sourceContainer, targetContainer) {
        const rect = sourceContainer.getBoundingClientRect();
        const centerX = rect.left + rect.width / 2;
        const centerY = rect.top + rect.height / 2 + 60; // Ниже основного текста
        
        // Создаем частицы для цифр
        const numbers = numberText.split('');
        const numberSpacing = 15;
        
        numbers.forEach((number, index) => {
            const offsetX = (index - (numbers.length - 1) / 2) * numberSpacing;
            
            // Создаем несколько частиц для каждой цифры
            const particlesPerNumber = 4;
            
            for (let i = 0; i < particlesPerNumber; i++) {
                setTimeout(() => {
                    const particle = document.createElement('div');
                    particle.className = 'critical-particle number';
                    particle.textContent = number;
                    particle.style.left = centerX + offsetX + 'px';
                    particle.style.top = centerY + 'px';
                    
                    // Более сильное разлетание для цифр
                    const angle = Math.random() * Math.PI * 2;
                    ...
                    // Разные задержки для эффекта фейерверка
